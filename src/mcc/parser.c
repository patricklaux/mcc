 ...
 * @return 词法单元
 */
Token *peek_tk(const Parser *parser, const size_t index) {
 ...
 * @return 下一词法单元
 */
Token *advance_tk(Parser *parser) {
    Token *token = peek_tk(parser, ++parser->t_index);
...
Token *consume(Parser *parser, const TokenKind expected) {
    const Token *tk = peek_tk(parser, parser->t_index);
    assert(expected, tk->kind);
    return advance_tk(parser);
}
...
int get_datatype(Parser *parser, const int basetype) {
    ...
    while (advance_tk(parser)->kind == TK_STAR) {
        datatype += PTR;
    }
    return datatype;
}
...
void parse_expr(Parser *parser, const int level, const int bp_index) {
    const Token *token = peek_tk(parser, parser->t_index);
...
void parse_stmt(Parser *parser, const int bp_index) {
    ...
    if (tk->kind == TK_IF) {
        ...
        *++parser->text = JZ; // 如果条件为零，跳转到下一语句，否则跳转到指定分支
        ...
        tk = peek_tk(parser, parser->t_index);
        if (tk->kind == TK_ELSE) {
            advance_tk(parser);
...
int parse_function_params(Parser *parser) {
    ...
    while (token->kind != TK_RIGHT_PAREN) {
        ...
        token = peek_tk(parser, parser->t_index); // 参数名
...
void parse_enum(Parser *parser) {
    const Token *token = advance_tk(parser);
    if (token->kind == TK_ID) {
        advance_tk(parser); // 跳过枚举名称
    }
...
void parser_parse(Parser *parser) {
    while (parser->t_index < parser->t_size) {
        ...
        const Token *p1 = peek_tk(parser, parser->t_index); // identifier
        assert(TK_ID, p1->kind);
        const Token *p2 = peek_tk(parser, parser->t_index + 1);
